...
export default class WhereClause {
  ...
  compoundOr(): CompoundQuery {
    if (!this.#currentExpression) {
      throw ORMError.queryError("Cannot use 'OR' without 'WHERE'");
    }
  ...
  compoundAnd(): CompoundQuery {
    if (!this.#currentExpression) {
      throw ORMError.queryError("Cannot use 'AND' without 'WHERE'");
    }
  ...
  #prepareSimpleCondition(condition: QueryExpression): string {
    if (!condition.condition) {
      throw ORMError.queryError("Condition not defined for simple expression");
    }
